...
async function main() {
	...
	const { results } = await executor.run(graph, {
		onProgress(e) {
			if (e.done) {
				...
				show(image.fromTensor(decodeLatent(config.taesd, lat), lat.w * f, lat.h * f, 3));
				previews++;
	...
	status = `done: ${final.width}x${final.height} in ${((llm.now() - t0) / 1000).toFixed(1)}s, ${frames - framesBefore} frames drawn, ${previews} previews`;
	show(final);
