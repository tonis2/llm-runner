// Depth Anything V2 (ViT-S/14) as a node: an image in, a depth map out.
//
// A DINOv2 encoder with four tapped layers and the DPT head, ported from the
// C3 `depth_anything.c3l`.
...
// A depth map (or, with `heightMap`, a tiling height map) of `src`, as an 8-bit
// grey image the size of `src`.
export function estimate(modelPath, src, opts = {}) {
	const heightMode = !!opts.heightMap;
	const res = opts.res || (heightMode ? 1036 : 518);
	...
	llm.print(`  depth: ${ow}x${oh} -> grid ${gw}x${gh}, model input ${gw * PATCH}x${gh * PATCH}`);
	...
	const model = new DepthAnything(modelPath);
	...
	if (heightMode) return gray(heightMap(values, src, ow, oh, opts), ow, oh);
	const { mn, mx } = normalize(values, 0, 1);
	llm.print(`  depth range ${mn.toFixed(3)}..${mx.toFixed(3)}`);
	return gray(values, ow, oh);
}
...
defineNodes('depth', {
	'depth.estimate': {
		...
		category: 'image',
		description: 'A grey depth map, near is bright. `height_map` makes a tiling relief map instead: depth high-passed by `detrend`, blended with the albedo\'s luminance (`albedo`, `flatten`) plus fine `detail`. `res` 0 is 518, or 1036 for a height map.',
		inputs: {
			image: 'IMAGE',
			path: 'PATH(depth)',
			res: 'INT=0',
		...
		run(i) {
			const t0 = llm.now();
			const img = estimate(i.path, i.image, {
				res: i.res, heightMap: i.height_map, detrend: i.detrend, detail: i.detail, albedo: i.albedo, flatten: i.flatten,
			});
			llm.print(`  [phase] depth: ${llm.since(t0)}`);
			return { image: img };
		},
	},
});
